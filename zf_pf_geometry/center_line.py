...
def create_2d_image_from_3d(center_plane_point, direction_2, direction_1, size_2d, image_3d):
    """
    Create a 2D image from a 3D image using the given plane and directions.
    """
    y_2d, x_2d = np.meshgrid(np.arange(size_2d[0]), np.arange(size_2d[1]), indexing='ij')
...
def radial_center_line(prox_dist_pos, y_pos, closed_image, num_angles=91):
    """
    Calculate the radial center line of the given image.
    """
...
def vertical_center_line(prox_dist_pos, closed_image):
    """
    Calculate the vertical center line of the given image.
    """
...
def get_Plane(point1, direction_pd, direction_dv, im_3d, centroid_3d):
    """
    Get the plane parameters for the given points and directions.
    """
...
def calculate_relative_positions(x_positions, y_positions):
    """
    Calculate the relative positions of the given coordinates.
    """
...
def center_line_session(mask_3d, df, scale_3d):
    """
    Main function to calculate the center line session.
    """
    ...
    try:
        ...
        logger.error(f"Error accessing path data: {e}")
        return None
